...
// ─── KasBridge interface (KAS ↔ iKAS bidirectional bridge on Igra) ───────────
// Contract: 0xb82c5524c5b5c055efb2F8f4AbCcE3173c504f2d
// iKAS is the native gas token of Igra (chain ID 38833).
// lockForExit locks iKAS in the bridge; relayers release KAS on L1 (Kaspa).
interface IKasBridge {
    /// @param kaspaAddress UTF-8 encoded Kaspa L1 destination address (e.g. "kaspa:qz...").
    function lockForExit(bytes calldata kaspaAddress) external payable;
}

/// @title FeeCollector
/// @notice Wraps the KAT Igra Bridge `lockForExit` and charges a fee on every
///         iKAS → KAS exit transaction. Fee is deducted from `msg.value` before
///         forwarding to the bridge contract.
///         Default fee rate: 0.75% (75 / 10000).
///
/// @dev iKAS is the native gas token of Igra (chain ID 38833), so all value
///      handling is identical to other native-token fee collectors.
contract FeeCollector {
    ...
    IKasBridge public immutable bridge;
    ...
    /// @param _bridge        KasBridge contract address on Igra.
    ...
    /// @notice Bridge iKAS from L2 to L1 KAS. Fee is deducted from `msg.value`;
    ///         the remainder is forwarded to the KasBridge contract.
    /// @param kaspaAddress  Kaspa L1 destination address (UTF-8, e.g. "kaspa:qz...").
    ///                      Max 100 bytes. Must start with "kaspa:".
    function bridgeToL1(string calldata kaspaAddress) external payable {
        if (msg.value == 0) revert InsufficientValue();

        // Validate Kaspa address: non-empty, starts with "kaspa:", max 100 bytes
        ...
        // KasBridge.lockForExit accepts raw UTF-8 bytes directly (no hex encoding)
        bridge.lockForExit{value: netValue}(addrBytes);
    }
